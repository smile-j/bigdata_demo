...
//    Class.forName("org.apache.spark.sql.hive.HiveSessionStateBuilder")
    System.setProperty("HADOOP_USER_NAME", "uhadoop")
    ...
    spark.sql(
      """
        |load data local inpath 'datas/spark_sql/product_info.txt' into table test_demo.product_info
      """.stripMargin)
    ...
    spark.sql(
      """
        |load data local inpath 'datas/spark_sql/city_info.txt' into table test_demo.city_info
      """.stripMargin)
